/* ══════════════════════════════════════════
   ...
   Import/Export, Pomodoro, Achievements, Sounds, CSV
══════════════════════════════════════════ */
...
function renderDueBanner() {
  let total = 0;
  decks.forEach(deck => deck.cards.forEach(card => { if (isDueToday(card)) total++; }));
...
function studyDueCards() {
  for (let i = 0; i < decks.length; i++) {
    if (decks[i].cards.some(c => isDueToday(c))) { startStudy(i, true); return; }
  }
}
...
function openEditDeck(i) {
  ...
  selectedColor = deck.color || '#2563eb';
...
Keep answers concise (under 15 words). Questions should be specific and testable.`;
  ...
  try {
    ...
    if (apiKey) {
      ...
      status.textContent = `✨ Created ${cards.length} cards with Smart Generator.`;
    }
    cards.forEach(c => {
      if (c.front && c.back) {
        tempCards.push({ front: c.front.trim(), back: c.back.trim(), hint: (c.hint || '').trim(), ease: 2.5, interval: 0, repetitions: 0, nextReview: null });
      }
    });
    renderPreview();
    ...
    stats.aiGenerations = (stats.aiGenerations || 0) + 1;
    saveAll();
    checkAchievements();
  } catch (err) {
    ...
    renderPreview();
    status.textContent = `⚠ Used Smart Generator fallback. Added ${fallback.length} cards.`;
...
function buildSmartCards(topic, count) {
  ...
  const templates = [
    { front: `What is ${capTopic}?`, back: `${capTopic} is a key concept to study.`, hint: 'Define in one sentence' },
    { front: `Why is ${capTopic} important?`, back: `It has major impact in its field.`, hint: 'Think impact' },
    { front: `Name one core idea in ${capTopic}.`, back: `A core idea involves its main principles.`, hint: 'Focus on fundamentals' },
    { front: `Give an example of ${capTopic}.`, back: `Use a real-world scenario to explain it.`, hint: 'Concrete example' },
    { front: `What is a common mistake about ${capTopic}?`, back: `Confusing related terms or skipping key details.`, hint: 'Think misconceptions' },
    { front: `How would you explain ${capTopic} to a beginner?`, back: `Use simple language and one analogy.`, hint: 'Keep it simple' },
    { front: `Which term is associated with ${capTopic}?`, back: `A central keyword that appears often in this topic.`, hint: 'Recall vocabulary' },
    { front: `What are two subtopics of ${capTopic}?`, back: `Break it into two smaller parts.`, hint: 'Split into components' },
    { front: `How can you test understanding of ${capTopic}?`, back: `Answer practice questions and explain from memory.`, hint: 'Active recall' },
    { front: `Summarize ${capTopic} in one line.`, back: `Core principles + practical application + examples.`, hint: 'One-line summary' }
  ];
...
function renderDecks(filterText = '', filterCat = '', filterTag = '') {
  ...
  const filteredEntries = decks.map((deck, idx) => ({ deck, idx })).filter(({ deck }) => {
  ...
  list.innerHTML = `<div class="deck-grid">${filteredEntries.map(({ deck, idx: realIdx }) => {
    ...
    return `
      <div class="deck-card ${isSelected ? 'selected' : ''}" style="--deck-color:${deck.color || '#2563eb'}" onclick="handleDeckCardClick(${realIdx}, event)">
        ${manageMode ? `<button class="deck-select" onclick="toggleDeckSelect(${realIdx}, event)"><span class="deck-select-box">${isSelected ? '✓' : ''}</span></button>` : ''}
...
function updateBulkUI() {
  ...
  count.textContent = `${selectedDeckIndices.size} selected`;
  document.body.classList.toggle('manage-mode', manageMode);
}

function handleDeckCardClick(deckIdx, event) { if (!manageMode) return; event.preventDefault(); toggleDeckSelect(deckIdx, event); }
...
function applyTagToSelected() {
  ...
  selectedDeckIndices.forEach(i => {
    ...
    if (!d.tags.some(t => t.toLowerCase() === tag.toLowerCase())) { d.tags.push(tag); changed++; }
  });
  if (changed) { saveAll(); refreshTagFilterOptions(); showToast(`Added tag "${tag}" to ${changed} deck${changed !== 1 ? 's' : ''} ✓`); }
  else showToast('Tag already present', 'error');
...
function removeTagFromSelected() {
  ...
  let changed = 0;
  selectedDeckIndices.forEach(i => {
    const d = decks[i]; if (!d || !Array.isArray(d.tags)) return;
    const before = d.tags.length;
    d.tags = d.tags.filter(t => t.toLowerCase() !== tag.toLowerCase());
    if (d.tags.length !== before) changed++;
  });
  if (changed) { saveAll(); refreshTagFilterOptions(); showToast(`Removed tag "${tag}" from ${changed} deck${changed !== 1 ? 's' : ''} ✓`); }
  else showToast('Tag not found', 'error');
  if (input) input.value = '';
  filterDecks();
}
...
function refreshTagFilterOptions() {
  ...
  select.innerHTML = ['<option value="">All tags</option>'].concat(tags.map(t => `<option value="${escHtml(t)}">${escHtml(t)}</option>`)).join('');
...
function deleteDeck(i) {
  ...
  if (deletedDeck?.id) Object.keys(cardHistory).forEach(k => { if (k.startsWith(`${deletedDeck.id}-`)) delete cardHistory[k]; });
  saveAll(); renderDecks(); updateStats();
...
function startStudy(index, dueOnly = false) {
  ...
  currentCards = dueOnly
    ? deck.cards.filter(c => isDueToday(c)).map((c) => ({ ...c, _origIdx: deck.cards.indexOf(c) }))
...
function answer(quality) {
  ...
  if (quality === 2) { stats.learned++; playSound('correct'); }
  else if (quality === 0) { playSound('wrong'); }
  const card = currentCards[currentIndex];
...
function startTimer() {
  ...
  timerInterval = setInterval(() => { timerSeconds++; document.getElementById('timer-count').textContent = timerSeconds; }, 1000);
}
...
function spawnConfetti() {
  ...
  for (let i = 0; i < 80; i++) {
    ...
    piece.style.cssText = `left:${Math.random()*100}vw;background:${colors[Math.floor(Math.random()*colors.length)]};width:${6+Math.random()*8}px;height:${6+Math.random()*8}px;animation-duration:${1.5+Math.random()*2}s;animation-delay:${Math.random()*0.8}s;border-radius:${Math.random()>0.5?'50%':'2px'};`;
...
function renderDeckPerformance() {
  ...
  container.innerHTML = decks.map(deck => {
    ...
    return `<div class="deck-perf-row"><span class="deck-perf-name">${escHtml(deck.name)}</span><div class="deck-perf-bar-wrap"><div class="deck-perf-bar-bg"><div class="deck-perf-bar-fill" style="width:${pct}%;background:${deck.color||'var(--accent)'}"></div></div></div><span class="deck-perf-pct">${deck.lastScore !== undefined ? pct + '%' : '—'}</span></div>`;
  }).join('');
}
...
function renderWeakCards() {
  ...
  container.innerHTML = weak.map(c => `<div class="weak-card-item"><span class="weak-card-q">${escHtml(c.front)}</span><span class="weak-card-rate">${c.rate}%</span></div>`).join('');
}
...
function importDeck(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!deck.name || !deck.cards.length) throw new Error('Invalid');
      decks.push(deck); saveAll(); renderDecks(); updateStats(); closeIOModal();
      showToast('Deck imported! 🎉');
    } catch { showToast('Invalid deck file', 'error'); }
  };
...
function checkImportFromURL() {
  ...
  const raw = params.get('deck'); if (!raw) return;
  try {
    const rawDeck = JSON.parse(decodeURIComponent(escape(atob(decodeURIComponent(raw)))));
    const deck = normalizeDeck(rawDeck);
    if (deck.name && deck.cards.length) { decks.push(deck); saveAll(); showToast(`Imported "${deck.name}" 🎉`); history.replaceState({}, '', location.pathname); }
...
function parseCSVText(text) {
  ...
  lines.forEach(line => {
    ...
    if (cleaned.length >= 2 && cleaned[0] && cleaned[1]) {
      cards.push({ front: cleaned[0], back: cleaned[1], hint: cleaned[2] || '', ease: 2.5, interval: 0, repetitions: 0, nextReview: null });
    }
  });
  return cards;
}
...
function importFromCSV() {
  ...
  if (!text) return showToast('Please paste some CSV data first!', 'error');
  const cards = parseCSVText(text);
  if (!cards.length) return showToast('No valid cards found. Check your format!', 'error');
  tempCards.push(...cards);
  renderPreview();
  const status = document.getElementById('csv-status');
  status.classList.remove('hidden');
  status.textContent = `✅ Added ${cards.length} cards! Go save your deck.`;
  stats.csvImports = (stats.csvImports || 0) + 1;
  saveAll(); checkAchievements();
  showToast(`${cards.length} cards imported from CSV! ✓`);
...
function importCSVFile(event) {
  const file = event.target.files[0]; if (!file) return;
  const reader = new FileReader();
  reader.onload = e => { document.getElementById('csv-input').value = e.target.result; showToast('File loaded! Click Import to add cards.'); };
  reader.readAsText(file);
  event.target.value = '';
}
...
function checkAchievements() {
  ...
  if (hour >= 0 && hour < 5) stats.nightStudy = true;
...
function showAchievementToast(ach) {
  ...
  document.getElementById('achievement-toast-name').textContent = ach.name + ' — ' + ach.desc;
  ...
  list.innerHTML = `
    <div class="ach-progress-bar-wrap">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <span style="font-size:14px;font-weight:600">${unlocked} / ${total} Unlocked</span>
        <span style="font-size:13px;color:var(--text-3)">${Math.round((unlocked/total)*100)}%</span>
      </div>
      <div class="progress-bar" style="height:8px;margin-bottom:0">
        <div style="width:${Math.round((unlocked/total)*100)}%;height:100%;background:var(--accent);border-radius:20px;transition:width 0.4s"></div>
      </div>
    </div>
      ...
      ${ACHIEVEMENTS.map(ach => {
        ...
        return `<div class="ach-card ${earned ? 'earned' : 'locked'}">
          ...
          <div class="ach-info">
            ...
            <div class="ach-desc">${earned ? ach.desc : 'Keep studying to unlock'}</div>
          </div>
          ${earned ? '<span class="ach-badge">✓</span>' : ''}
        </div>`;
      }).join('')}
    </div>
  `;
...
══════════════════════════════════════ */
const POMO_MODES = {
  focus: { label: 'Focus Time', duration: 25 * 60, color: '#ef4444' },
...
let pomosCompletedToday = 0;

function setPomoMode(mode) {
  pomoMode = mode;
  pomoRunning = false;
  clearInterval(pomoInterval);
  pomoRemaining = POMO_MODES[mode].duration;
  document.querySelectorAll('.pomo-tab').forEach(t => t.classList.remove('active'));
  document.getElementById(`tab-${mode}`).classList.add('active');
  document.getElementById('pomo-label').textContent = POMO_MODES[mode].label;
  document.getElementById('pomo-btn').textContent = '▶ Start';
  document.querySelector('.pomo-ring-fill').style.stroke = POMO_MODES[mode].color;
  updatePomoDisplay();
}
...
function togglePomo() {
  if (pomoRunning) {
    ...
    pomoInterval = setInterval(() => {
      pomoRemaining--;
      if (pomoMode === 'focus') pomoTotalFocusSeconds++;
      updatePomoDisplay();
      if (pomoRemaining <= 0) { clearInterval(pomoInterval); pomoRunning = false; onPomoComplete(); }
    }, 1000);
  }
}
...
function onPomoComplete() {
  ...
  if (pomoMode === 'focus') {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification('🍅 Pomodoro Complete!', { body: 'Time for a break. Great work!' });
    } else if ('Notification' in window && Notification.permission !== 'denied') {
      Notification.requestPermission();
    }
    if (pomosCompletedToday % 4 === 0) setPomoMode('long');
    else setPomoMode('short');
  } else {
    pomoSessionNum++;
    setPomoMode('focus');
    document.getElementById('pomo-session').textContent = `Session ${pomoSessionNum} of ${pomoGoal}`;
  }
}

function resetPomo() {
  pomoRunning = false; clearInterval(pomoInterval);
  pomoRemaining = POMO_MODES[pomoMode].duration;
  document.getElementById('pomo-btn').textContent = '▶ Start';
  updatePomoDisplay(); playSound('click');
}
...
function updatePomoDisplay() {
  ...
  const pct = pomoRemaining / POMO_MODES[pomoMode].duration;
  document.getElementById('pomo-ring-fill').style.strokeDashoffset = 603 * (1 - pct);
  document.getElementById('pomo-total-time').textContent = Math.floor(pomoTotalFocusSeconds / 60) + 'm';
}

function updatePomoGoal(val) {
  pomoGoal = parseInt(val);
  document.getElementById('pomo-goal').textContent = val;
}
...
document.addEventListener('keydown', e => {
  ...
  closeHelp(); closeIOModal(); closeCSVModal();
});
